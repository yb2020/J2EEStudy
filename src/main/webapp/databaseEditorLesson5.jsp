...
<head>
  ...
  <script>
    function addOne() {
      ...
      xmlhttp.open("POST","<%=request.getContextPath()%>/ajaxServlet?value=" + document.getElementById("test").innerText);
      xmlhttp.send();//发送请求
      xmlhttp.onreadystatechange = ()=> {//readyState每次改变就会执行onreadystatechange
        if(xmlhttp.readyState == 4){//readyState是请求状态
          if(xmlhttp.status == 200){
            var testObj = eval("(" + xmlhttp.responseText + ")");
            document.getElementById("test").innerText = testObj.text
          }
        }
      }
    }
